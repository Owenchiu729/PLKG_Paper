...
Typical usage
    python3 csi_recorder_v2.py --max 5000 --label tx
    python3 csi_recorder_v2.py --sec 10 --sync_stop --label rx
"""
...
def next_csv_filename(port: str, label: str = "") -> str:
    """Create an incremented CSV filename based on port AND optional label."""
    if os.name == "posix":
        tag = port.rsplit("/", 1)[-1].lower()
    else:
        tag = port.lower().replace(":", "")
        
    # FIX: Correctly handle label logic here
    if label:
        base_name = f"{DEFAULT_BASENAME}_{tag}_{label}"
    else:
        base_name = f"{DEFAULT_BASENAME}_{tag}"
        
    pattern = os.path.join(DATA_PATH, f"{base_name}_*{CSV_EXT}")
    ...
    if existing:
        ...
        try:
            ...
            idx = 1
    return os.path.join(DATA_PATH, f"{base_name}_{idx:03d}{CSV_EXT}")
...
def reader_thread(
    ...
    peer_ap: str | None,
    peer_sta: str | None,
    label: str = ""   # FIX: Added label parameter
):
    """
    One reader per port.
    ...
    """
    # FIX: Pass label to filename generator
    out_path = next_csv_filename(port, label)
...
def main():
    ...
    ap.add_argument("--label", type=str, default="", help="Append label to filename (e.g., tx, rx, pi1)")
    ...
    for p in ports:
        t = threading.Thread(
            ...
            # FIX: Added args.label to the arguments tuple
            args=(p, args.baud, args.max, args.peer_ap, args.peer_sta, args.label),
            daemon=True,
        )
        t.start()
        threads.append(t)
